...
MedMirror 是一套可复用的医疗大模型呈现差异评测工作流（CLI，clone 即用），结论可回溯原文：用自主开发方法，在限定预算内自主开展受控评测、追问并交付可复核实验发现包。

## 评委快速入口（1 分钟）

**这是什么**：一套可复用的呈现差异评测工作流——在限定预算内自主评测「三家模型对同一合成病例的治疗路径呈现差异」，受控协议采集 27 条基线原文 + 3 个追问结果（含 1 条零正文，如实记录），产出可复核的描述性差异报告；**不输出未经专业复核的医学 Bias 结论**。
