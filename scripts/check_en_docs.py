...
**残余边界（如实声明；主人 2026-09-15 拍板「冻结现状」）**：本闸**不做**完整 Markdown/HTML
解析——本仓零运行时依赖，手写解析到此为止。它防的是**常规漂移与常见遮蔽**：声明被删或写弱、
被挪进代码块（含未闭合/波浪线/缩进/引用块内）、HTML 注释与隐藏元素、行内代码、伪链接与链接
title、字段漏译与重名、测试数漂移。**刻意构造**的写法**不在覆盖内**（双谱系评审 A 第 7 轮
仍在构造出这一类，故明确划界而非继续加固）：嵌套引用前缀、列表标记后再缩进、URL 内含括号的
链接、引用式链接定义、外链样式表定义的隐藏类、JS 运行时隐藏、图片 alt、`<template>`/`<svg>`
内容，以及锚点跨行拆开。这些靠人读对照兜底。
CI 全绿 = 上述四类规则在覆盖范围内成立，**不等于**「声明一定可见、数字一定为真、全仓无漂移」。
