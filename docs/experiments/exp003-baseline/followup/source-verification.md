...
- 三态判定：**可定位**（著录信息与真实文献相符，允许漏写期号等常见简化；本文档「可定位」均指**文献著录层面**的定位，区别于 followup 触发规则的「原文可定位」＝引文命中父回答）/ **部分相符**（文献或相近文献真实存在，但作者、期刊、年份、卷期或标题等著录项有出入）/ **无法定位**（按自述著录查无此文献，或自述过于模糊无法精确核对）。部分相符与无法定位的分界判据：自述著录无法整体匹配到单篇文献时，若其作者、研究名或机构等**关键著录项可与某真实文献精确重合**（如 D11 的 Jiang X），记部分相符；自述无著录项重合、但其作者与研究的描述可唯一定位到某真实研究（如 D8 之于 CAPITAL，匹配发生在描述层而非著录项层），亦记部分相符；仅主题相近而无著录项重合、亦无法唯一定位（如 S2、S4），记无法定位。
- 与项目既有来源状态阶梯的关系：本三态是对 `specs/calibration.md` §3「被提到 / 存在 / 支持」三态中**「来源存在」一档**的操作化细分（对应 `specs/review/boundaries.md` 的「被提到 ≠ 可识别 ≠ 已核实」阶梯；两套措辞中「存在」与「可识别」均指来源定位到单篇真实文献的层面、「支持」与「已核实」均指内容核实完成，侧重不同）；「可定位」「部分相符」均**未达**「已核实」，「已核实」保留给 #5 专业复核。boundaries.md 对材料包来源「本研究不自动判定」的约定针对复核包语境；本轮查证是主人 2026-09-12 拍板的描述性研究行为（issue #29），不改变该约定。
...
- 判定：所指研究可唯一定位为 CAPITAL（描述层匹配：作者团队+研究类型+主题三处相符，与 D11 的著录项重合同归部分相符，见 :6 判据），但期刊与年份著录不符 → 部分相符。模型对其规模的自述（「样本量有限、单中心为主」）与 CAPITAL 的多中心设计是否相符属内容判断，不在本查证范围。
