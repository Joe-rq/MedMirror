...
> **项目定位**：MedMirror 是一套可复用的医疗大模型呈现差异评测工作流（CLI；评测研究者 clone 即用，环境与四闸见根 README），结论可回溯原文。
...
- **分组呈现差异**（词项命中层面，提取器 offline-rules-v2，已按缩窄口径定标）：
...
uv run python scripts/report_exp003.py --input docs/experiments/exp003-baseline/result/trials.jsonl --output /tmp/replay-judge --repeats 3   # 无密钥离线重放；analysis.md/.json 与 derived-v3 字节一致（目录内 extraction-diff.md 由 diff_extractions.py 另行生成，不在重放范围）。extractions.jsonl 不在此列：每次回放自身确定，但自 #50 起每行附 paths_digest 词表签名（2026-09-13 引入），不与 2026-09-10 冻结的 derived-v3 快照字节相同
```

### 边界

提取器为确定性规则（offline-rules-v2），已按缩窄口径定标（1 条完整标注 + 9 条裁决 + 14 条抽查信任扩展，后者未经逐字段独立标注，`specs/calibration/fp-fn-report.md`）；提及矩阵是词项命中，不判医学质量。单病例小样本，不作模型间稳定性结论。
...
- 医学复核人尚未落实（issue #5），两层复核材料已备、第一批已于 2026-09-12 发出（登记见 `specs/review/record.md`），意见未回流；**没有任何候选已获得专业复核结论**。
- 提取器定标按缩窄口径完成（14 条信任扩展未经独立标注，局限随行）；专业复核仍待 #5。
...
- 供应商账单绝对值核对已完成（`specs/calibration/bill-check.md`，2026-09-12 主人登录三家控制台核对，合计 0.4746 元）。

## 附 B：可复现性与工程护栏

- **284 项离线测试**（`uv run pytest`）+ CI 四闸（ruff format / ruff check / pytest / check-manifests）全绿，另有两条文档卫生闸（`scripts/check_docs.py` / `scripts/check_en_docs.py`）同段执行；无密钥、无网络可全量重放。
- 派生报告（`analysis.md`/`.json`）**字节级可重放**（任务一定位命令 3；`extractions.jsonl` 的口径以该命令注释为准）；复核包附件可幂等重生成并比对（`uv run python scripts/gen_review_attachments.py --output-dir /tmp/att-replay && diff -r /tmp/att-replay specs/review/attachments`，字节一致即通过；避免直接覆盖入库附件）。
...
2. 提取定标为缩窄口径（14 条信任扩展未经独立标注）——语义声明按该口径限定。
3. 单病例（合成）小样本（3 重复）——不支撑模型间稳定性或 Bias 结论，仅描述性观察。
4. glm 追问零正文的供应商侧成因、step 截断的完整来源清单——需新实验，明确不在本件范围。
5. 三家实际账单与估算的绝对值核对——已完成（`specs/calibration/bill-check.md`，2026-09-12，合计 0.4746 元）。
