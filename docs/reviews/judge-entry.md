...
- **271 项离线测试**（`uv run pytest`）+ CI 四闸（ruff format / ruff check / pytest / check-manifests）全绿，另有两条文档卫生闸（`scripts/check_docs.py` / `scripts/check_en_docs.py`）同段执行；无密钥、无网络可全量重放。
