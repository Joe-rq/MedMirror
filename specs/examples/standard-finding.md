...
状态：**数值已刷新（2026-09-13，来源 `docs/experiments/exp003-baseline/derived-v3/`，提取器 offline-rules-v2）——待主人确认文末拍板清单 4 项**。数值层已按缩窄口径定标（1 条完整标注 + 9 条口头裁决 + 14 条抽查后接受机器结果；14 条信任扩展未经逐字段独立标注，局限声明见 `specs/calibration/fp-fn-report.md`，在一切引用中保留）。`OWNER_TO_CONFIRM` 为主人拍板项，均已附 AI 建议供圈选。本示范属于定标样本，不当作独立验证数据。
...
`OWNER_TO_CONFIRM`（AI 建议：沿用 specs/calibration.md §1 已确认表述）：
...
`OWNER_TO_CONFIRM`（AI 建议：采纳下条假设与反驳材料，或改写）：

- 假设：显式询问能提高对应路径的呈现率，但不必然改变其支持条件。
- 支持材料：上表 n/N 与引文（tcm_mirror 下 tcm 提及 8/8 完整 + 截断文内 1/1；neutral 下 0/9）。
- 反驳材料（AI 草拟，`OWNER_TO_CONFIRM`）：更简解释是提示内容不对称本身——tcm_mirror 提示明确要求中医方案，呈现率变化可直接归因于任务指令，无需诉诸模型内在倾向；且 deepseek 在 western_mirror 下仍主动提及中医 1/3 次，与"仅镜像才提及"的强版本不符。
...
- 已观察事实（AI 草拟，`OWNER_TO_CONFIRM`）：西医路径在全部 24 条完整回答中被提及 24/24；中医路径仅在被明确询问时被提及（tcm_mirror 完整 8/8），中性问法 0/9；deepseek 在 western_mirror 下主动提及中医 1/3 次，step、glm 均为 0；追问轮 glm 两次零正文、step 截断、deepseek 完整（followup 独立分组）。
- 待验证解释：见上节假设。
- 无法判断的问题（AI 草拟，`OWNER_TO_CONFIRM`）：呈现差异的医学合理性（归 #5 复核）；模型间/组内稳定性（3 次重复不足）；差异成因（训练数据 vs 提示敏感性，本设计不可区分）。
...
| # | 拍板项 | AI 建议 |
|---|---|---|
| 1 | 研究问题表述（§研究问题） | 沿用 specs/calibration.md §1 原文 |
| 2 | 待复核医学问题记录方式（§待复核医学问题） | 必核 10 条编号 + 指向 `specs/review/form-candidates.md`，不复制主张全文 |
| 3 | 解释假设与反驳材料（§解释假设） | 采纳草拟假设 + 奥卡姆反驳（提示不对称的更简解释 + deepseek 主动提及反例） |
| 4 | 结论边界草拟（§结论边界） | 按上节草拟定稿，或改写 |

拍板后：清除全部 `OWNER_TO_CONFIRM` 标记，状态行转「已确认」，本文成为 specs/calibration.md 结构标准的实例正本。
