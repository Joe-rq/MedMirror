...
**补记（2026-09-15，评审轮）：**双谱系评审第一谱系（codex gpt-5.6-luna）两轮 + 自修一轮，全部闭环。R1（5 P1 + 7 P2 + 1 P3）实锤三处：①英文 README 的回放比对用 `for` 循环跑 `diff`，退出码只取最后一条——首个不一致会被吞掉（已改 `cmp` + `&&`）；②闸门可见性可被绕过（未闭合围栏/波浪线围栏/HTML 属性/行内代码/缩进代码/未闭合注释/隐藏元素/图片与转义伪链接/无首尾竖线表格行/表外伪字段）；③英文入口漏「中／西医变体不是医学等价对照」这条协议边界（补入并升为第 4 条声明锚点）。R2 复验后可见性解析改为**块级状态机**（围栏+缩进代码+注释+隐藏元素），残余边界（外链样式表、JS 运行时隐藏、SVG/template）写进 docstring 如实声明。另按 R2 补：CaseSpec 文档把「红线不可绕过」改成如实说明机械覆盖范围（**登记闸只比对词表摘要，变体改动不进登记闸**——已登记缺口）、回放命令输出措辞、CI 表述三处统一（README/onboarding/.cnb.yml 头注释）。**最终计数：四闸 271 + check_calibration + 两条文档闸全绿；本闸回归 35 例（含 6 个 subtest）**；测试数 264→271 同源刷新。第二谱系（MiniMax-M3）结果续记。

**补记（2026-09-15，谱系 B 与终值）：**第二谱系 MiniMax-M3 两轮：首轮（边想边测长稿、未给分级结论）中两条可操作项已闭环（锚点匹配域剥离链接目标、残余边界措辞改精确），其「`specs/example` 单数」疑点经两方各自实测**撤回**（HEAD、`origin/main`、文件系统三处均为 `specs/examples` 复数）；第二轮判**可合并**（0 P0/P1/P2），并逐条实测确认：数字可溯源、离线回放收窄声明与实测一致、闸门无未声明的假绿路径、CI 六 stage 无 `&&` 遮蔽。**最终计数：测试 272（本闸回归 36 例 + 6 subtest）、四闸 + check_calibration + 两条文档闸全绿。**

**补记（2026-09-15，R4/R5 终验后定稿值）：**谱系 A 定向核验两轮：R4 确认 R3 四条闭环，另指出两条 P1（闭合围栏行内不得再有内容——CommonMark；未闭合 `<style>`/`<script>` 未剥离）与一条 P3（字段名重复不拦）；R5 确认 R4 全部闭环，另指出一条 P1（4 空格缩进的围栏行被误判为闭合——CommonMark 只允许 ≤3 空格）与两条 P2（Markdown 引用式注释 `[//]: # (…)` 未剥离；changelog 终值滞后）；R6 确认 R5 四项闭环，另指出一条 P1（引用块内缩进代码）与两条 P2（链接 title、带前缀的引用式注释）。全部已修复并加回归。**本件最终口径：测试 284（本闸回归 48 例 + 13 subtest）、四闸 + 两条文档闸 + check_calibration 全绿。**
**补记（2026-09-15，可见性收口决定）：**谱系 A 第七轮核验确认前轮修复有效，但仍构造出**新一族**合法 Markdown 遮蔽（嵌套引用前缀、列表标记后缩进、URL 内含括号的链接、引用式链接定义），并注意这些构造需**刻意写出**。**主人 2026-09-15 拍板「冻结现状」**：本闸定位为防常规漂移与常见遮蔽，刻意构造类缺口**如实声明为残余边界**（已写进 `scripts/check_en_docs.py` 模块 docstring），人读对照为最后一层；不引入 Markdown 解析依赖（本仓零运行时依赖），也不继续手写加固。代价如实记账：A 第 7 轮的 3 条（1 P1 + 2 P2）以「已声明边界」结案，未逐条修复。
**补记（2026-09-15，合并、关闭与镜像同步）：**MR #58 由主人合并于 `a4ea1bc`（快进合并），CNB push 流水线 `cnb-4n8-1k2iam1l2` success；issue #51 已先回复处理结果再关闭（completed，回读验证通过）。GitHub 镜像经主人授权同步至 `a4ea1bc` 并逐一验证：匿名 clone 实测六闸全绿（284 例）、新闸在镜像上绿（4 条声明锚点 / 9 个字段 / 2 对互链 / 测试数 284 六份同值）、四条声明锚点与 284 tests 在匿名可读的 README.en.md 上均可达、本次 13 个变更文件密钥复扫干净——素材稿 checklist 第 12 条「承诺须在镜像上成立」对本件已满足。主人两次裁决记录在本条前两段：**收窄「字节一致」声明**（不改已外送证据、不撤评审守卫）与**冻结可见性加固**（刻意构造类缺口如实声明为残余边界）。
