...
**补记（2026-09-15，R4 终验后定稿值）：**谱系 A 定向终验确认 R3 四条闭环，另指出两条新 P1（围栏闭合判定过宽——尾部带文本的围栏行（闭合线后还有字符）被误当作闭合；未闭合 `<style>`/`<script>` 未剥离）与一条 P3（字段名重复不拦），已全部修复并加回归。**本件最终口径：测试 277（本闸回归 41 例 + 10 subtest）、四闸 + 两条文档闸 + check_calibration 全绿。**
