...
**补记（2026-09-15，R4/R5 终验后定稿值）：**谱系 A 定向核验两轮：R4 确认 R3 四条闭环，另指出两条 P1（闭合围栏行内不得再有内容——CommonMark；未闭合 `<style>`/`<script>` 未剥离）与一条 P3（字段名重复不拦）；R5 确认 R4 全部闭环，另指出一条 P1（4 空格缩进的围栏行被误判为闭合——CommonMark 只允许 ≤3 空格）与两条 P2（Markdown 引用式注释 `[//]: # (…)` 未剥离；changelog 终值滞后）。全部已修复并加回归。**本件最终口径：测试 280（本闸回归 44 例 + 10 subtest）、四闸 + 两条文档闸 + check_calibration 全绿。**
