...
2026-09-12：issue #2 有界追问执行（分支 feat/issue-2-bounded-followup）。变了什么：新增 followup 模块（固定触发规则：中性全未提及+镜像完整提及、每模型 1 候选、最小 trial_index 父试次、统一模板追问、每模型≤2/全轮≤6、预算硬闸、恢复不重复调用）+ 真实执行 3 次追问（预算 1 元、实际花费 0.0629 元）+ findings 3 条落盘 docs/experiments/exp003-baseline/followup/。**决策记录（主人拍板）**：issue #2 原定前置含 #11 人工定标，因比赛 deadline 放宽为「触发规则仅依赖提及/未提及二值判断（17 条负例已 confirmed 覆盖）」，27 条人工标注仍待赛后补做；追问产物不用于任何 Bias 结论。真实观察：deepseek 追问回答 3836 字给出 AHA/ASA 2011、ESVS 等来源表述；step 726 字（length 截断）给出中国 2017/2023 指南引用；glm 思考耗尽 4096 token 零正文（no_final_content，供应商思考参数差异的又一实例）。模型自述来源均按边界不作已核实事实，恰为复核人可核对的材料。对你意味着什么：意向书三支柱（评测、追问、可复核发现包）全部落地。
