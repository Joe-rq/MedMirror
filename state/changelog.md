...
2026-09-15（补记）：issue #51 双谱系评审第一谱系两轮闭环。变了什么：`scripts/check_en_docs.py` 的可见性解析由「黑名单去块」改为**块级状态机**（围栏 ```/~~~ 未闭合到文末、4 空格/Tab 缩进代码、HTML 注释未闭合到文末、任意标签带 hidden/display:none/visibility:hidden、style/script 整块），行内代码与图片/转义伪链接不再算命中，表格行识别支持 GFM 省首尾竖线（含两侧都省）与引用块表格，字段名只认表格内的行；英文 README 的回放比对由 `for ... diff`（退出码只取最后一条，会吞首个不一致）改为 `cmp` + `&&`；CaseSpec 文档把「协议红线（不可绕过）」改为如实说明**机械覆盖范围**（登记闸只比对词表摘要，变体改动靠纪律与评审——已登记缺口）；CI 表述三处统一。为什么变：codex gpt-5.6-luna R1 判「修改后合并」（5 P1 + 7 P2 + 1 P3），R2 复验仍有 5 P1。**决定记录**：①可见性不追求穷尽（不引入 Markdown/HTML 解析依赖），残余边界写进脚本 docstring 如实声明——CI 绿 ≠ 声明一定可见，人读对照仍是最后一层；②英文入口补入的第 4 条声明（变体非等价对照）升级为机械锚点，中英两侧同守。对你意味着什么：改文档时闸门更严（声明必须出现在可见正文里，藏进代码块/注释/隐藏元素一律红灯）；`configs/cases/` 的变体安全目前**没有**机械闸，只有纪律与评审——这是已知缺口，是否补代码守卫由主人定。
2026-09-15（补记二 · 终值）：**测试 284**（本闸回归 48 例 + 13 subtest）、四闸与两道文档闸 + check_calibration 全绿。评审链：谱系 A codex gpt-5.6-luna R1–R7（含四轮定向核验）与谱系 B MiniMax-M3 R1–R2 的发现全部闭环；上文「18 例回归」为首次交付时的数字，本行覆盖之（中间值 274/38 亦为过程值，以本行为准）。
2026-09-15（补记三 · 合并与关闭）：issue #51 收口。变了什么：MR #58 合并入库（`a4ea1bc`，快进），issue #51 回复处理结果后按 completed 关闭；GitHub 镜像在主人授权下同步至同一提交并验证——匿名 clone 六闸全绿（284 例）、新闸在镜像上绿、四条声明锚点与 284 tests 匿名可达、变更文件密钥复扫干净。为什么变：本件已交付（英文入口 + 中英一致性闸），素材稿 checklist 第 12 条要求对外承诺在被镜像指向的仓库上成立，故同步后逐项实测而非只推不验。对你意味着什么：国际读者从 GitHub 拉到的就是这个状态；后两条收口决定（收窄字节一致声明、冻结可见性加固）同时生效——「CI 绿 ≠ 声明一定可见」已写进闸门 docstring，人读对照仍是最后一层。
